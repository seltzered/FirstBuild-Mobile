...
- (void)drawRect:(CGRect)rect {
    ...
    NSArray* gradColors = [NSArray arrayWithObjects:
                           ...
                           (id)[UIColorFromRGB(0x782A25) colorWithAlphaComponent:0.8].CGColor, // make this an argument? property most likely
                           nil];
    ...
    CGContextDrawRadialGradient(context, gradient, center, (rect.size.height)/4.0, CGPointMake(rect.size.width + 5, rect.size.height), (rect.size.height)/2, kCGGradientDrawsAfterEndLocation); // how to find rounded corners?
    //CGContextSaveGState(context);
    // to bottom left corner
    CGContextDrawRadialGradient(context, gradient, center, (rect.size.height)/4.0, CGPointMake(-5, rect.size.height), (rect.size.height)/2, kCGGradientDrawsAfterEndLocation);
    // to top left corner
    CGContextDrawRadialGradient(context, gradient, center, (rect.size.height)/4.0, CGPointMake(-5, 0), (rect.size.height)/2, kCGGradientDrawsAfterEndLocation);
    // to top right corner
    CGContextDrawRadialGradient(context, gradient, center, (rect.size.height)/4.0, CGPointMake(rect.size.width + 5, 0), (rect.size.height)/2, kCGGradientDrawsAfterEndLocation);
    //top
